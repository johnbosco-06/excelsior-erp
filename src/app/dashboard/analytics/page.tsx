...
import { computeCGPA, computeRisk, getGrade, type RiskScore, type CGPAResult } from "@/lib/cgpa"
import type { AuthUser } from "@/lib/auth"
import type { Database } from "@/lib/supabase"
import {
  BarChart3, AlertTriangle, TrendingUp, TrendingDown,
  Users, Activity, Loader2, ChevronDown, ChevronUp,
  Shield, ShieldAlert, ShieldX, Eye
} from "lucide-react"
...
export default function AnalyticsPage() {
  ...
  // Compute analytics for all students in section
  const runAnalytics = async () => {
    ...
    for (const student of students) {
      ...
      const { data: attData } = await supabase.from('attendance')
        .select('status').eq('student_id', student.id)
      ...
      const { data: marksData } = await supabase.from('marks')
        .select('marks_obtained, max_marks').eq('student_id', student.id)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const { data: marksData } = await supabase.from('marks')
        .select('marks_obtained, max_marks').eq('student_id', profile.id)
      const totalObtained = marksData?.reduce((s, m) => s + Number(m.marks_obtained), 0) ?? 0
      const totalMax      = marksData?.reduce((s, m) => s + Number(m.max_marks), 0) ?? 0
      const avgMarksPct   = totalMax > 0 ? Math.round(totalObtained / totalMax * 100) : 0
  ...
  const avgCGPA = analytics.length
    ? (analytics.reduce((s, a) => s + a.cgpaResult.cgpa, 0) / analytics.length).toFixed(2)
    : '—'
  const avgAtt = analytics.length
    ? Math.round(analytics.reduce((s, a) => s + a.attendancePct, 0) / analytics.length)
    : 0

  return (
    <div className="p-6 space-y-6">
      <div>
        <span className="font-mono text-xs text-primary">// SECTION: ANALYTICS</span>
        <h1 className="text-2xl font-bold tracking-tight mt-1">Analytics & CGPA</h1>
        <p className="font-mono text-xs text-muted-foreground mt-1">
          {isStudent ? 'Your CGPA, grade history and risk assessment'
            : 'Section-wise CGPA, at-risk detection and performance analytics'}
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          {myData.risk.flags.length > 0 && (
            <div className={`rounded-lg border p-4 space-y-2 ${RISK_COLORS[myData.risk.riskLevel].bg} ${RISK_COLORS[myData.risk.riskLevel].border}`}>
              <p className={`font-mono text-xs font-bold ${RISK_COLORS[myData.risk.riskLevel].text}`}>
                ⚠ Risk Flags
              </p>
          ...
          {myData.cgpaResult.semesters.length === 0 && (
            <div className="bg-card border border-border rounded-lg p-12 text-center">
              <BarChart3 className="w-8 h-8 text-muted-foreground mx-auto mb-3" />
              <p className="font-mono text-sm text-muted-foreground">No marks entered yet</p>
            </div>
          )}
        </div>
      )}
      ...
      {isStudent && loading && (
        <div className="bg-card border border-border rounded-lg p-12 text-center">
          ...
          <p className="font-mono text-sm text-muted-foreground mt-3">Computing CGPA...</p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          {analytics.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {(['SAFE','WATCH','AT_RISK','CRITICAL'] as const).map(level => {
                const count = riskCounts[level] ?? 0
                const pct   = Math.round(count / analytics.length * 100)
                const { bg, border, text, icon: Icon } = RISK_COLORS[level]
                return (
                  <div key={level} className={`rounded-lg border p-4 ${bg} ${border}`}>
                    <div className="flex items-center gap-2 mb-2">
                      <Icon className={`w-4 h-4 ${text}`} />
                      <span className={`font-mono text-xs font-bold ${text}`}>{level}</span>
                    </div>
                    <p className={`text-3xl font-bold ${text}`}>{count}</p>
                    <p className="font-mono text-xs text-muted-foreground">{pct}% of section</p>
                  </div>
                )
              })}
            </div>
                    ...
                    <div key={a.profile.id}>
                      ...
                      {isExp && (
                        <div className="px-6 pb-4 space-y-3 border-t border-border bg-accent/20">
                          ...
                          {a.cgpaResult.semesters.map(sem => (
                            <div key={sem.semester} className="space-y-1">
                              <p className="font-mono text-xs text-primary mt-2">
                                Sem {sem.semester} — GPA: {sem.gpa.toFixed(2)} ({sem.credits}cr)
                              </p>
                              {sem.results.map(r => (
                          ...
                          {a.cgpaResult.semesters.length === 0 && (
                            <p className="font-mono text-xs text-muted-foreground pt-2">No marks entered yet</p>
                          )}
                        </div>
                      )}
                    </div>
          ...
          {analytics.length === 0 && !loading && (
            <div className="bg-card border border-border rounded-lg p-12 text-center">
              <BarChart3 className="w-8 h-8 text-muted-foreground mx-auto mb-3" />
              <p className="font-mono text-sm text-muted-foreground">Select a section and click Run Analytics</p>
            </div>
          )}
        </div>
      )}
